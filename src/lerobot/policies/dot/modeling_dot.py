...
class DOT(nn.Module):
    def __init__(self, config: DOTConfig):
        ...
        self.camera_names = sorted(
            [k for k in self.config.input_shapes if k.startswith("observation.image")]
        )
        if len(self.camera_names) > 0:
            ...
            self.projections["images"] = add_lora_to_backbone(
                backbone, rank=config.lora_rank
            )
            self.n_features += len(self.camera_names) * self.config.n_obs_steps

        # For the rest of the inputs the projection is a simple linear layer
        for state in ["state", "environment_state"]:
            obs_state = f"observation.{state}"
            if obs_state in self.config.input_shapes:
                self.projections[state] = nn.Linear(
                    self.config.input_shapes[obs_state][0], self.config.dim_model
                )
                self.n_features += self.config.n_obs_steps

        self.projections_names = sorted(list(self.projections.keys()))
        ...
        decoder_pos_inf = self.decoder_pos[
            : self.decoder_pos.shape[0]
            + self.config.inference_horizon
            - self.config.train_horizon
        ]
        ...
        self.inputs_pos_emb = nn.Parameter(
            torch.empty(1, self.n_features, self.config.dim_model)
        )
        nn.init.uniform_(
            self.inputs_pos_emb,
        ...
        self.action_head = nn.Linear(
            self.config.dim_model, self.config.output_shapes["action"][0]
        )
    ...
    def _process_inputs(self, batch):
        ...
        for state in self.projections_names:
            batch_state = f"observation.{state}"
...
class DOTPolicy(nn.Module, PyTorchModelHubMixin):
    name = "dot"

    def __init__(
        self,
        config: DOTConfig | None = None,
        dataset_stats: dict[str, dict[str, Tensor]] | None = None,
    ):
        super().__init__()
        self.config = config or DOTConfig()
        self.model = DOT(self.config)
        self._setup_normalizers(dataset_stats)
        ...
        action_weights = (
            config.alpha ** torch.arange(self.config.inference_horizon).float()
        )
        ...
        loss_weights = torch.ones(
            self.config.train_horizon + self.config.n_obs_steps - 1
        )
    ...
    def _setup_normalizers(self, dataset_stats):
        self.normalize_inputs = Normalize(
            self.config.input_shapes,
            self.config.input_normalization_modes,
            dataset_stats,
        )
        self.normalize_targets = Normalize(
            self.config.output_shapes,
            self.config.output_normalization_modes,
            dataset_stats,
        )
    ...
    def _update_observation_buffers(
        self, buffer_name: str, observation: Tensor
    ) -> Tensor:
    ...
    def _prepare_batch_for_inference(
        self, batch: dict[str, Tensor]
    ) -> dict[str, Tensor]:
        batch = self.normalize_inputs(batch)

        # Resize and stack all images
        if len(self.model.camera_names) > 0:
            batch["observation.images"] = torch.stack(
                [self.resize_transform(batch[k]) for k in self.model.camera_names],
                dim=1,
            )  # bs, n_cam, c, h, w

        # Update observation queues for all inputs and stack the last n_obs_steps
        for k in self.model.projections_names:
            batch[f"observation.{k}"] = self._update_observation_buffers(
                f"observation.{k}", batch[f"observation.{k}"]
            )
    ...
    def select_action(self, batch: dict[str, Tensor]) -> Tensor:
        ...
        if self.step % self.predict_every_n == 0:
            ...
            self.last_action = self.unnormalize_outputs({"action": actions_pred})[
                "action"
            ]
        else:
    ...
    def forward(self, batch: dict[str, Tensor]) -> dict[str, Tensor]:
        ...
        for k in (
            [f"observation.{k}" for k in self.model.projections_names]
        ...
        if len(self.model.camera_names) > 0:
            ...
            for k in self.model.camera_names:
            ...
            batch["observation.images"] = torch.stack(
                [batch[k] for k in self.model.camera_names], dim=2
        ...
        if self.state_noise is not None:
            for k in self.model.projections_names:
                if "image" not in k:
                    batch[f"observation.{k}"] += (
                        torch.rand_like(batch[f"observation.{k}"]) * 2 - 1
                    ) * self.state_noise

        actions_hat = self.model(batch)

        loss = F.l1_loss(batch["action"], actions_hat, reduction="none")
    ...
    def from_pretrained(cls, model_path, *args, **kwargs):
        ...
        policy = super().from_pretrained(model_path, *args, **kwargs)
...
def create_sinusoidal_pos_embedding(num_positions: int, dimension: int) -> Tensor:
    ...
    div_term = torch.exp(
        torch.arange(0, dimension, 2, dtype=torch.float)
